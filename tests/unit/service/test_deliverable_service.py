...
class TestDeliverableService:
    ...
    @patch('src.service.deliverable_service.get_database_repository')
    @pytest.mark.parametrize("pdf_text,expected_name", [
        ("Student: Jane Smith\nHomework", "Jane Smith"),
        ("Submitted by: Bob Johnson\nDate", "Bob Johnson"),
        ("Author: Charlie Davis\nIntro", "Charlie Davis"),
        ("Alice Brown\nCS101 Assignment", "Alice Brown"),
    ])
    def test_extract_student_name_patterns(
        self, mock_get_repo: MagicMock, pdf_text: str, expected_name: str
    ) -> None:
    ...
    def test_extract_name_with_openai_success(self, mock_get_repo: MagicMock, mock_post: MagicMock) -> None:
        ...
        mock_response.json.return_value = {
            "choices": [{"message": {"content": "John Smith"}}]
        }
    ...
    @pytest.mark.parametrize("exception,expected_log", [
        ...
        (__import__('requests').exceptions.Timeout("Timeout"), "OpenAI API request timed out"),
    ])
    def test_extract_name_with_openai_exceptions(
        self, mock_get_repo: MagicMock, mock_post: MagicMock, 
        exception: Exception, expected_log: str
    ) -> None:
    ...
    def test_extract_name_with_openai_cleans_result(
        ...
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "choices": [{"message": {"content": "Name: John Smith"}}]
        }
        mock_post.return_value = mock_response
    ...
    def test_upload_deliverable_with_name_extraction_logging(
        ...
        with patch.object(DeliverableService, 'extract_student_name_from_pdf',
                         return_value=("Alice Johnson", "extracted text")):
            service = DeliverableService()
            result = service.upload_deliverable(
                "assignment_id", "homework.pdf", b"pdf content",
                "pdf", "application/pdf", extract_name=True
            )
    ...
    def test_upload_deliverable_non_pdf_with_extract_name(self, mock_get_repo: MagicMock) -> None:
        ...
        service = DeliverableService()
        deliverable_id = service.upload_deliverable(
            "assignment_id", "document.txt", b"text content",
            "txt", "text/plain", extract_name=True
        )
    ...
    def test_upload_deliverable_assignment_not_found(self, mock_get_repo: MagicMock) -> None:
        ...
        with pytest.raises(ValueError, match="Assignment with ID test_id not found"):
            service.upload_deliverable("test_id", "submission.pdf", b"content", 
                                      "pdf", "application/pdf")
    ...
    def test_upload_multiple_deliverables_success(self, mock_get_repo: MagicMock) -> None:
        ...
        with patch.object(DeliverableService, 'extract_student_name_from_pdf',
            ...
            deliverable_ids = service.upload_multiple_deliverables(
                "assignment_id", files, extract_names=True
            )
    ...
    @patch('src.service.deliverable_service.logger')
    @patch('src.service.deliverable_service.get_database_repository')
    def test_upload_multiple_deliverables_with_errors(
        self, mock_get_repo: MagicMock, mock_logger: MagicMock
    ) -> None:
        ...
        service = DeliverableService()
        deliverable_ids = service.upload_multiple_deliverables(
            "assignment_id", files, extract_names=False
        )
    ...
    def test_upload_multiple_deliverables_assignment_not_found(self, mock_get_repo: MagicMock) -> None:
        ...
        with pytest.raises(ValueError, match="Assignment with ID test_id not found"):
            service.upload_multiple_deliverables(
                "test_id", [("file.pdf", b"content", "pdf", "application/pdf")], False
            )
    ...
    def test_update_deliverable_success(self, mock_get_repo: MagicMock) -> None:
        ...
        result = service.update_deliverable(
            "deliverable_id", student_name="Updated Name",
            mark=8.55, certainty_threshold=0.95
        )
    ...
    @patch('src.service.deliverable_service.get_database_repository')
    @pytest.mark.parametrize("mark,error_msg", [
        (15.0, "Mark must be between 0.0 and 10.0"),
        (-10.0, "Mark must be between 0.0 and 10.0"),
    ])
    def test_update_deliverable_invalid_mark(
        self, mock_get_repo: MagicMock, mark: float, error_msg: str
    ) -> None:
    ...
    @patch('src.service.deliverable_service.get_database_repository')
    @pytest.mark.parametrize("certainty,error_msg", [
    ...
    @patch('src.service.deliverable_service.PdfReader')
    @patch('src.service.deliverable_service.get_database_repository')
    def test_extract_student_name_from_pdf_calls_openai(self, mock_get_repo: MagicMock, mock_pdf_reader: MagicMock) -> None:
    ...
    @patch('src.service.deliverable_service.get_database_repository')
    @pytest.mark.parametrize("filename,content_type,expected_valid,expected_error", [
    ...
    def test_validate_file_format(
        self, mock_get_repo: MagicMock, filename: str, content_type: str,
        expected_valid: bool, expected_error: str
    ) -> None:
    ...
    def _create_mock_deliverable(
        self, student_name: str = "John Doe", mark: float | None = None
    ) -> DeliverableModel:
