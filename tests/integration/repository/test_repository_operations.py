...
class TestRepositoryOperations:
    ...
    def test_assignment_listing_with_pagination(self, repo: DatabaseRepository, cleanup_assignments: List[str]) -> None:
        for i in range(10):
            assignment_id = repo.create_assignment(
                name=f"List Test {i:02d}",
                confidence_threshold=0.50 + i * 0.05
            )
            cleanup_assignments.append(assignment_id)
    ...
    def test_file_listing_by_assignment_and_type(self, repo: DatabaseRepository, cleanup_assignments: List[str]) -> None:
    ...
    def test_cascade_deletion_assignment_with_files_and_deliverables(self, repo: DatabaseRepository) -> None:
        ...
        file_id = repo.store_file(
            assignment_id, "rubric.pdf", b"rubric", "application/pdf", "rubric"
        )
