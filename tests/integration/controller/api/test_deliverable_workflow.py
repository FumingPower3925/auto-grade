...
class TestDeliverableWorkflow:
    ...
    def setup_method(self) -> None:
        ...
        response = self.client.post("/assignments", json={
            "name": "Deliverable Test Assignment",
    ...
    def test_bulk_deliverable_upload(self) -> None:
        ...
        response = self.client.post(
            f"/assignments/{self.assignment_id}/deliverables/bulk",
            files=files,
    ...
    @pytest.mark.parametrize("mark,expected_status", [
    ...
    @pytest.mark.parametrize("certainty,expected_status", [
        (0.0, status.HTTP_200_OK),
    ...
    def test_deliverable_certainty_validation(self, certainty: float, expected_status: int) -> None:
        ...
        response = self.client.patch(
            f"/deliverables/{deliverable_id}",
            json={"certainty_threshold": certainty}
        )
        assert response.status_code == expected_status
